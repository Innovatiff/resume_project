...
export const GET = withHandler(async (req) => {
  ...
  const me: MeResponse = {
    user: { uid: doc.uid, email: doc.email, displayName: doc.displayName, language: doc.language ?? "en", city: doc.city, province: doc.province, createdAt: doc.createdAt },
...
export const PATCH = withHandler(async (req) => {
  ...
  const body = await readJson<{ language?: string; city?: string; province?: string; displayName?: string }>(req);
