...
- **Install.** `npm run build:extension` writes `extension/dist` (load it unpacked from `chrome://extensions` with Developer mode on) and `public/downloads/orvenic-extension.zip`, which the app's Extension page links to. Both are committed so a customer can install without building. Once the Chrome Web Store listing is live, set `NEXT_PUBLIC_EXTENSION_STORE_URL` to it and the page offers Add to Chrome instead. `extension/store/` holds the listing text and screenshots for the submission.
