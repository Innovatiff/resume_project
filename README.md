...
**Free path.** Firebase's Spark plan covers sign-in and Firestore at this stage, and Render's free web service runs the app with no short function timeout. `render.yaml` in the repo is a Render Blueprint: New → Blueprint → this repo, then fill in the secrets it asks for. Free instances sleep after 15 minutes without traffic, and the visitor who wakes one waits half a minute in front of Render's own loading page, which is expensive when the traffic is paid for.

A free scheduler at [cron-job.org](https://cron-job.org) requests `https://orvenic.com/api/health` every 10 minutes to keep the instance awake. A plain `GET` there touches no database and no external service, so the ping costs only instance hours. Render allows 750 of those per workspace per month against roughly 730 hours in a month, so pinging around the clock fits but leaves no margin, and a second free service would put the workspace over and suspend both. The job is therefore restricted to waking hours. Two things it cannot cover: the gap while a deploy restarts the service, so deploy outside the hours ads are running; and a pinger that fails quietly, so leave its failure notifications on.

This is a workaround for the free tier, not a fix. Render's Starter plan removes sleeping altogether, and with it the deploy gap and the hours ceiling; move to it (or to Firebase App Hosting) once there is revenue. The Anthropic API itself is pay-per-use (a few cents per package) and needs prepaid credit.
...
- **Install.** `npm run build:extension` writes `extension/dist` (load it unpacked from `chrome://extensions` with Developer mode on) and `public/downloads/orvenic-extension.zip`, which the app's Extension page links to. Both are committed so a customer can install without building. The Chrome Web Store listing is the route customers use; `brand.extensionStoreUrl` in `src/lib/content.ts` carries it (overridable with `NEXT_PUBLIC_EXTENSION_STORE_URL`), and the Extension page offers Add to Chrome whenever it is set, keeping the zip as the fallback. `extension/store/` holds the listing text and screenshots.
- **Access.** Every extension call except identification needs a live 30-Day Pass or Landed; when the pass ends the popup says so and offers renewal, and it works again the moment a new pass is active, with nothing to reconnect. Keys stay valid until disconnected or the account is deleted.
