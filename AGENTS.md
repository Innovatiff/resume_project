...
- Browser extension: `extension/` (MV3, TypeScript bundled by `extension/build.mjs` into the committed `extension/dist` and `public/downloads/orvenic-extension.zip`). It authenticates with keys from `/app/extension` (`src/lib/store/extension-keys.ts`, hashed) against `/api/extension/*`. Field handling is decided in `src/lib/extension/fields.ts`, which is unit-tested: legal attestations and self-identification are flagged and never filled, salary asks get the pay report as a hint, and the content script has no code path that clicks submit. Keep it that way. `npm run e2e:extension` drives it against the ATS look-alikes in `e2e/fixtures/ats/`.
